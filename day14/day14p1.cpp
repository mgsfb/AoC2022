...
// 1 landed, -1 done, -2 illegal
int DropSand (std::vector<std::vector<int>> &board, int R, int C)
{

    if (R >= board.size()-1 || C>=board[0].size()-1 || C < 0) return -1; //
...
//            Pt.R+=Rmargin;
            Pt.C-=minC;
            maxR = std::max(maxR,Pt.R);
            maxC = std::max(maxC,Pt.C);
        }
    }

    std::cout << "C range: " << minC <<", " << maxC << "  R range: " << minR << ", " << maxR << std::endl;
    sC -=minC;  //shift the sand, too.
